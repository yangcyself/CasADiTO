    ...
    IpoptCalculatedQuantities *ip_cq)
{
    ...
    static casadi_int n_arg = parse_newx_n_in();
    _arg[n_arg-1] = x;

    AUTO_SET_UP_WORKING_MEM(parse_newx, 0, x_n_res, x_);
    parse_newx(_arg, x_res, x_iw, x_w, 0);
    newx = Eigen::MatrixXd::Zero(parse_newx_sparsity_out(0)[0],
                    parse_newx_sparsity_out(0)[1]);
    compCCS_fillDense(parse_newx_sparsity_out(0),
                        x_res[0], newx);
